...
export class PurchaseBillDetailComponent {
  ...
  compId !: number;
  branchId !: number;
  constructor(private purchaseBillService: PurchaseBillService, private activatedRoute: ActivatedRoute,
  ...
  ngOnInit() {
    ...
    this.purchaseBillService.fetchPurchaseBillDetailForInvoice(billNo, this.compId, this.branchId).subscribe(data => {
