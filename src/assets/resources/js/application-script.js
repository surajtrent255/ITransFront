$(function(){
    ...
    var customerRecordsPerPage = parseInt($('.customerPaginationData span.endIndex').text());
    var adjustmentRecordsPerPage = parseInt($('.adjustmentPaginationData span.endIndex').text());
    var productRecordsPerPage = parseInt($('.productPaginationData span.endIndex').text());
    var pricingTypeRecordsPerPage = parseInt($('.pricingTypePaginationData span.endIndex').text());
    var pricingSeqRecordsPerPage = parseInt($('.pricingSeqPaginationData span.endIndex').text());  
    var groupComboRecordsPerPage = parseInt($('.groupComboPaginationData span.endIndex').text());  
    var prodAttrRecordsPerPage = parseInt($('.productAttrPaginationData span.endIndex').text()); 
    var custAttrRecordsPerPage = parseInt($('.customerAttrPaginationData span.endIndex').text());    
    ...
    /*--- Draggable Groups --- */
    createGroupTree("#organizationHierarchy", orgGroupType, ".jstree-drop-org", "/groups/organization/edit");
    createGroupTree("#productHierarchy", productGroupType, ".jstree-drop-product", "/groups/product/edit");
        ...
        function  createGroupTree(groupTreeId, groupType, dropTarget, editUrl){
            ...
            "json_data" : {
                "ajax" : {
                    ...
                    "url" :  function(node){
                          if( node == rootNode){
                            temp_node = node;
                            return "/groups/hierarchies/display?type=" + groupType + "&level=" + initialLoadHierarchy + "&rand=" + new Date().getTime();
                          } else {
                            temp_node = 0;
                            return "/groups/hierarchies/display?unid=" + node.attr('unid') + "&type=" + groupType + "&level=" + nonInitialLazyLoad + "&rand=" + new Date().getTime();
                          }
                        },
                    "success" : function (hierarchy) {
                        if(temp_node == rootNode){
							...
							var data = [];
							$.each(hierarchy.hierarchy.hierarchies,function(iterator,group){
								var state = (group.hasChildren == 0) ? "leaf":"closed";
								data.push({
									"data":group.name,
									"attr":{"id":group.id,"unid":group.unid,
									"parentId":group.parentId,"startDate":group.startDate,
									"endDate":group.endDate,"nameOnly":group.nameOnly},
									"state":state});
							});
							return data;
                        }
            ...
            "dnd":{
                "drop_finish" : function (data) {
                    ...
                    if(data.r.hasClass('customer-id-drop')){
                        ...
                        data.r.parents('tr').siblings('.customer-name-field-row').children('.customer-name').text(data.o.attr('nameonly'));
                        
                    }
                    if(data.r.attr('unid') != "") {
                        data.r.attr("unid", data.r.attr("unid")+':'+data.o.attr('unid'));
                    } else {
                        data.r.attr("unid", data.o.attr('unid'));
                    }
                    if(data.r.hasClass('tokenInputText')){
                    	var addedToken = "<li itemId='"+data.o.attr('unid')+"'>"+data.o.attr('nameonly')+" <a href='#' class='deleteToken'><img src='resources/images/deleteInputToken.png'/></a></li>";
                    ...
                    if(data.r.hasClass('groupComboTypeDivWrapper')){
                        ...
                        if($('#groupComboTypeTable tbody tr#'+dataUnid).length == 0){
                        	...
                        	var typeRow = "<tr id="+data.o.attr('unid')+"><td class='colAttr organizationId'>"+data.o.attr('nameonly')+"</td><td>" +
                            "<select id ='groupComboModifier"+data.o.attr('unid')+"' class='inputText groupComboModifierSelect'>" +
                    ...
                    if(data.r.hasClass('custAdvSrcGrpName')){
                        $('#custAdvanceSearch input#custAdvSrcGrpId').val($('#custAdvanceSearch input#custAdvSrcGrpId').val()+","+data.o.attr('unid'));
                    }
					
					if(data.r.hasClass('proSrcProductGrpTxt')){
                        $('#proSrcProductGrp').val($('#proSrcProductGrp').val()+","+data.o.attr('unid'));
                    }
                    else if(data.r.hasClass('adjSerProdGrpIdTxt')){
                        $('#adjSerProdGrpId').val($('#adjSerProdGrpId').val()+","+data.o.attr('unid'));
                    }
					
                    if(data.r.hasClass('proSrcCustId')){
                        $('#prodAdvanceSearch input#proSrcCustId').val(data.o.attr('unid'));
                        $('#proSrcCustName').text(data.o.attr('nameonly'));
                    }
                    if(data.r.hasClass('adjSerOrgIdTxt')){
                        $('#adjSerOrgId').val($('#adjSerOrgId').val()+","+data.o.attr('unid'));
                    }
                    if(data.r.hasClass('customerTableDataLayout')){
                        //var url="/customers/adv-search";

                        var startIndex = parseInt($('.customerPaginationData span.startIndex').text());
                        var endIndex = parseInt($('.customerPaginationData span.endIndex').text());
                        var totalRecords = (endIndex - startIndex) + 1;
                        var params={'custGrpId':data.o.attr('unid'),
            ...
            moveParams = { "startDate":data.rslt.o.attr("startdate"),
        ...
        $('#productHierarchy ul li a').live('click',function(e){
            e.stopPropagation()
            $('#organizationHierarchy ul a.jstree-clicked').removeClass('jstree-clicked');
        });
        ...
        $('#cutNode').live('click',function(){
            ...
            cutCopyParams = { "startDate":$('.jstree-clicked').parent().attr('startdate'),
            ...
            if ($('.jstree-clicked').parents('.hierarchyContainer').attr('id') == 'organizationContainer') {
                $.jstree._reference($("#organizationHierarchy")).cut($('.jstree-clicked'));
        ...
        $('#pasteNode').live('click',function(){
            ...
            if(($('.jstree-clicked').parents('.hierarchyContainer').attr('id') == 'organizationContainer') && (cutCopyParams.type == 'product')){
                popAlert('Product cant be pasted in organizations.');
                return false;
            }
            if(($('.jstree-clicked').parents('.hierarchyContainer').attr('id') == 'productContainer') && (cutCopyParams.type == 'organization')){
        ...
        $('#nodePaste').live('click',function(){
          ...
          if ($('#chooseOptionForPaste input#addAsSibling').is(':checked') && $('.hierarachy-Div>ul>li>a').hasClass('jstree-clicked')) {
              $('#nodePaste').parent().siblings('.siblingCheck').show();
              return true;
          }
          if (!($('#chooseOptionForPaste input#addAsSibling').is(':checked')) && !($('#chooseOptionForPaste input#addAsChild').is(':checked'))) {
              $('#chooseOptionForPaste span.optionCheck').show();
              return false;
          }
          if ($('#chooseOptionForPaste input#addAsSibling').is(':checked')) {
              refNode =  $('li[unid="'+$('.jstree-clicked').parents().attr('parentId')+'"]');
          ...
          if(cutCopyParams.isCopy == true){
              var url;
              if ($('#chooseOptionForPaste input#addAsSibling').is(':checked') && data == 'organizationContainer') {
                  url = '/groups/organization/add-sibling';
                  cutCopyParams.unid = $('#chooseOptionForPaste input#unid').val();
              }
              if ($('#chooseOptionForPaste input#addAsSibling').is(':checked') && data == 'productContainer') {
                  url = '/groups/product/add-sibling';
                  cutCopyParams.unid = $('#chooseOptionForPaste input#unid').val();
              }
              if ($('#chooseOptionForPaste input#addAsChild').is(':checked') && data == 'organizationContainer') {
                  url= '/groups/organization/add-child';
                  cutCopyParams.unid = $('#chooseOptionForPaste input#unid').val();
              }
              if ($('#chooseOptionForPaste input#addAsChild').is(':checked') && data == 'productContainer') {
        ...
        $('.editGroupCombo').live('click',function(){
            ...
            $('#editGroupComboForm textarea#commentText').val($(this).parent().siblings('.groupComboCommentText').html())
            $('#editGroupComboForm input#addGenCustProduct').removeAttr('checked');
            var type = $(this).parent().siblings('.groupComboType').html();
            if(type == 1){
                $('#editGroupComboForm input#groupComboEditOrganization').attr("checked","checked");
                $('#editGroupComboForm div.groupComboTypeDivWrapper').addClass('jstree-drop-org');
                $('#editGroupComboForm div.groupComboTypeDivWrapper').removeClass('jstree-drop-product');
            }
            else{
                $('#editGroupComboForm input#groupComboEditProduct').attr("checked","checked");
                $('#editGroupComboForm div.groupComboTypeDivWrapper').addClass('jstree-drop-product');
                $('#editGroupComboForm div.groupComboTypeDivWrapper').removeClass('jstree-drop-org');
            }
            var groupid = {'groupIds':$(this).parent().siblings('.groupComboIds').html()};
                 ...
                 success: function(str){
                     ...
                     $(str.groupComboInfoRes.groupComboInformation).each(function(){
                         ...
                         if(name=='GENERIC_CHECKED'){
                             $('#editGroupComboForm input#addGenCustProduct').attr('checked','checked');
                         }else{
                             var temp = unid;
                             if(unid.indexOf('+')>=0 || unid.indexOf('-')>=0){
                                 unid = unid.substring(0, unid.length -1);
                             }
                             var typeRow = "<tr id="+unid+"><td class='colAttr organizationId'>"+ name +"</td><td>" +
                                     "<select id ='groupComboModifier"+unid+"' class='inputText groupComboModifierSelect"+unid+"'>" +
                                     "<option class='gands"+unid+"' value='1'>Groups &nbsp; Subgroups</option>" +
                                     "<option class='groups"+unid+"' value='2'>Groups only</option>" +
                                     "<option class='subgroups"+unid+"' value='3'>subGroups only</option></select></td></tr>";
                             $('#groupComboTypeTable tbody').append(typeRow);

                             temp = temp.substring(temp.length-1,temp.length);
                             if(temp == '+'){
                                 $('.organizationId').siblings().children('.groupComboModifierSelect'+unid).val('1');
                                 //$('.organizationId').siblings().children('.groupComboModifierSelect'+unid).children('.gands'+unid).attr('selected',true);
                             } else if(temp=='-') {
                                 $('.organizationId').siblings().children('.groupComboModifierSelect'+unid).val('3');
                                  //$('.organizationId').siblings().children('.groupComboModifierSelect'+unid).children('.subgroups'+unid).attr('selected',true);
                             } else {
                                 $('.organizationId').siblings().children('.groupComboModifierSelect'+unid).val('2');
    ...
    /*---- header Settings Popups Script --*/
        $('#configurationLink').live('click',function(){
            ...
            $('html, body').animate({scrollTop:0}, 'slow');
            $('.pageOverlay').fadeIn('fast');
            showTblIdTreeCheck=$('#defaultConfigurationData').children('#showTableIdTreeCheckDefault').text();
            if(showTblIdTreeCheck=='checked'){
                $('#showTableIdTreeCheck').attr('checked',showTblIdTreeCheck);
            }else{
                $('#showTableIdTreeCheck').removeAttr('checked');
            }
            if($('#defaultConfigurationData').children('#showTableIdDialogCheckDefault').text()=='checked'){
                $('#showTableIdDialogCheck').attr('checked','checked');
            }else{
                $('#showTableIdDialogCheck').removeAttr();
            }
            $('#effDateFrom').val($('#defaultConfigurationData').children('#effDateFromDefault').text());
            $('#effDateTo').val($('#defaultConfigurationData').children('#effDateToDefault').text());
            if($('#defaultConfigurationData').children('#getProductsMasterNameCheckDefault').text()=='checked'){
        ...
        function prepopulateCustomerData(popupId, tagId) {
            ...
            $('#val_'+tagId).children('span').each(function(){
                if($(this).attr('class') == 'custId') {
                    $('#'+popupId+' span.'+$(this).attr('class')).text($(this).text());
                } else if($(this).attr('class') == 'custGroupId') {
                    $('#'+popupId+' .'+$(this).attr('class')).attr("unid", $(this).text());
                    var li = $('<li>').attr('itemid',$(this).text()).text($('#'+tagId+' .customerOrganizationName').text());
                    var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                    li.append(an);
                ...
                success: function(str){
                    ...
                    $(str.customerAttributes.attributes).each(function(){
                        ...
                        if(actionType=='edit'){
                            ...
                            createNewAttr('createNewCustomerPopup','newcustomerattr','custAttributes');
                        }
                    });
                },
                error:function(XMLHttpRequest, textStatus,errorThrown){
                ...
                success: function(str){
                    ...
                    $(str.productAttributes.attributes).each(function(){
                        ...
                        if(actionType=='edit'){
                            ...
                            createNewAttr('createNewProductPopup','newproductattr','prodAttributes');
                        }
                    });
                },
                error:function(XMLHttpRequest, textStatus,errorThrown){
        ...
        function createNewAttrDiv(attrDivId,data){
            var colName = typeof(data["columnName"]) != 'undefined' ? data["columnName"] : "";
            var colType= typeof(data['columnType']) != 'undefined' ? data['columnType'] : "";
        ...
        function createNewAttr(popupId,tagId,attrId){
            ...
            tableId.html("");
            $('#val_'+tagId).children().each(function(){
                var attrNameCol = $('<td>').addClass("colAttr").append($(this).children('.attrDesc').text());
                attrNameCol.attr('colName',$(this).children('.attrName').text());
                attrNameCol.attr('colType',$(this).children('.attrType').text());
                if($(this).children('.attrType').text()=='Cust. Numeric'){ //TODO: check  if for product attribute also
                    clsNumeric="numeric";
                    msgSpan='<span class="validationMsg"><br/>You must enter valid data for attribute.</span>';
                }else{
                    clsNumeric='';
                    msgSpan="";
                }
                var inputVal = $('<input>').addClass("inputText"+" "+clsNumeric).attr("type", "text");
        ...
        function populateAttributes(popupId, tagId, attrId,newAttrId) {
            var tableId = $('#'+popupId + ' .'+attrId);
            tableId.html("");
            $('#val_'+tagId).children('div').each(function(){
                var attrDesc=$('#val_'+newAttrId).children('#'+$(this).children('.attrName').text().toLowerCase()).children('.attrDesc').text();
                var attrType=$('#val_'+newAttrId).children('#'+$(this).children('.attrName').text().toLowerCase()).children('.attrType').text();
                ...
                var inputVal = $('<input>').addClass("inputText"+" "+clsNumeric).attr("type", "text").attr("value", $(this).children('.attrVal').text());
                var attrValCol = $('<td>').append(inputVal);
                attrValCol.append(msgSpan);
                var row = $('<tr class="attrRow">').append(attrNameCol);
                row.append(attrValCol);
        ...
        function prepopulateProductData(popupId, tagId) {
            ...
            $('#val_'+tagId).children('span').each(function(){
                if($(this).attr('class') == 'prodUnid') {
                    $('#'+popupId+' td.'+$(this).attr('class')).text($(this).text());
                } else if($(this).attr('class') == 'prodGroupId') {
                    $('#editProduct .inputText.prodGroupId').siblings('.tokenList').empty();
                    var li = $('<li>').attr('itemid',$(this).text()).text($(this).siblings('.productGroups').text());
                    var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                    li.append(an);
                    $('#editProduct .inputText.prodGroupId').siblings('.tokenList').append(li);
                    $('#'+popupId+' .'+$(this).attr('class')).attr("unid", $(this).text());
                } else if ($(this).attr('class') == 'prodOrgId') {
                    $('#editProduct ul#organizationId').empty();
                    var li = $('<li>').attr('itemid',$(this).text()).text($(this).text());
                    var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                    li.append(an);
        ...
        function prepopulateAdjustmentData(popupId, tagId) {
            ...
            $('#val_'+tagId).children('span').each(function(){
                if($(this).attr('class') == 'adjUnid') {
                    ...
                    $('#editAdjustment .adjOperation').html($('#pricingTypeOperationDropDown option:selected').html());
                } else if($(this).attr('class') == 'adjCustName' && $(this).text() != "") {
                    $('#'+popupId+' td.customer-name').text($(this).text());
                } else if($(this).attr('class') == 'adjCustId' && $(this).text() != ""){
                    ...
                    var li = $('<li>').attr('itemid',$(this).text()).text($(this).siblings('.adjProdGroups').text());
                    var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                    li.append(an);
                    $('#editAdjustment ul#productId').append(li);
                    $('#'+popupId+' td input#editAdjProdGroups').attr("unid", $(this).text());
                } else if($(this).attr('class') == 'adjOrgIds' && $('#'+popupId+' td input#editAdjCustId').val() == "") {
                    $('#'+popupId+' td input#editAdjOrgs').attr("unid", $(this).text());
                } else if($(this).attr('class') == 'adjOrgs' && $('#'+popupId+' td input#editAdjCustId').val() == "") {
                    var temp = $(this).siblings('.adjOrgIds').text();
                    $('#'+popupId+' .'+$(this).attr('class')).attr("unid",temp);
                    var li = $('<li>').attr('itemid',$(this).siblings('.adjOrgIds').text()).text($(this).text());
                    var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                    li.append(an);
            ...
            jQuery.ajax({
                ...
                success: function(str){
                    ...
                    if(objectId.indexOf(":") != -1) {
                        $('#adjustmentsRecordsTableEditPopup #editAdjProdGroups').val(str.adjustments.adjustments[0].objectName);
                    }
                    var orgIds = $('#val_adj_'+unid +" .adjOrgIds").text();
                    if(orgIds.indexOf(":") != -1) {
                        $('#adjustmentsRecordsTableEditPopup #editAdjOrgs').val(str.adjustments.adjustments[0].organizationName);
                    }
                },
                error:function(XMLHttpRequest, textStatus,errorThrown){
                    // TODO: need to show error message
    ...
    /*--- New Customer/Adjustment/Product popup Call ---*/
        $('.createNewTableRecord').live('click',function(){
            ...
            $('#'+recordTypeId+'Popup .inputText').each(function(){
                if($(this).attr('unid')!=='undefined' || $(this).attr('unid')!==false){
                    $(this).attr('unid','');
                }
            });
            if($('#'+recordTypeId+'Popup .inputText').attr('unid')!=='undefined' || $('#'+recordTypeId+'Popup .inputText').attr('unid')!==false){
    ...
    /*----- Configur Table Scripts ----- */
        ...
        $('.removeFilterPrompt').live('click',function(){
            $(this).parents('.configNavTableWrapper').siblings('.filterListWrapper').children('.filtersDisplayList').empty();
            $(this).parents('.configNavTableWrapper').siblings('.filterListWrapper').children('.filtersDisplayList')
                        .siblings().children('.italicText').text('');
        });

        /*--- adding filters while clicking -- */
        $('.filterList .filterLink').live('click',function(){
            $(this).parents('.configNavTableWrapper').siblings('.filterListWrapper').children('.filtersDisplayList').html("");
            var parentId = $(this).parents('.configTableDropdown').attr('id');
            $('.'+parentId+'DisplayList').siblings().children('.italicText').text('filtered by');
            var filterText = $(this).html();
            var filterMarkup = '<li><a href="#" class="removeFilterLink"><img src="resources/images/removeFilter.png"></a><span>'+filterText+'</span></li>';
            $('.configTableDropdown, .allFiltersList').hide();
            $('.'+parentId+'DisplayList').append(filterMarkup);
            return false;
        });
        $('.removeFilterLink').live('click',function(){
            if($(this).parents('.fleft').hasClass('customerTableFiltersDisplayList') == true){
            	$('#customerNewFilter').attr('search','off');
                $('.customerTableFiltersDisplayList').siblings().children('.italicText').text('');
                getCustomerFirstPage(false);
            }
            if($(this).parents('.fleft').hasClass('adjustmentTableFiltersDisplayList') == true){
            	$('#adjustmentNewFilter').attr('search','off');
                $('.adjustmentTableFiltersDisplayList').siblings().children('.italicText').text('');
                getAdjustmentFirstPage(false);
            }
            if($(this).parents('.fleft').hasClass('productsTableFiltersDisplayList') == true){
            	$('#newProductFilter').attr('search','off');
            	$('.productsTableFiltersDisplayList').siblings().children('.italicText').text('');
        ...
        $('.recordsPerPage').live('click',function(){
            ...
            if($(this).parents('ul').attr('id') == 'customerConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.customerPaginationData span.endIndex').html()));
                $('#recordPerPageForm .submitBtn').attr('id','customerRecordsPerPage');
            }else if($(this).parents().attr('id') == 'adjustmentConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.adjustmentPaginationData span.endIndex').html()));
                $('#recordPerPageForm .submitBtn').attr('id','adjustmentRecordsPerPage');
            }else if($(this).parents().attr('id') == 'productConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.productPaginationData span.endIndex').html()));
                $('#recordPerPageForm .submitBtn').attr('id','productRecordsPerPage');
            }else if($(this).parents().attr('id') == 'pricingTypeConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.pricingTypePaginationData span.endIndex').html()));
                $('#recordPerPageForm .submitBtn').attr('id','pricingTypeRecordsPerPage');
            }else if($(this).parents().attr('id') == 'pricingSeqConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.pricingSeqPaginationData span.endIndex').html()));
                $('#recordPerPageForm .submitBtn').attr('id','pricingSeqRecordsPerPage');
            }else if($(this).parents().attr('id') == 'groupComboConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.groupComboPaginationData span.endIndex').html()));
                $('#recordPerPageForm .submitBtn').attr('id','groupComboRecordsPerPage');
            } else if($(this).parents().attr('id') == 'prodAttrConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.productAttrPaginationData span.endIndex').html()));
                $('#recordPerPageForm .submitBtn').attr('id','prodAttrRecordsPerPage');
            } else if($(this).parents().attr('id') == 'custAttrConfigList'){
                $('#recordPerPageForm input#records').val(parseInt($('.customerAttrPaginationData span.endIndex').html()));
        ...
        $('.master-data-table-container .recordCategory .minimizeLink').live('click',function(){
            ...
            if(maxlinkActiveId=='customerRecordTable'){
                ...
                addLayoutConfiguration('list-item','MINIMIZED_ADJUSTMENT_RECORD_TABLE');
        ...
        $('.shiftToDisplay').live('click',function(){
            var destinationDiv = $(this).parents('.colControls').siblings('.colDisplayed').children('select');
            var optionsTomove = $(this).parents('.colControls').siblings('.colNotDisplayed').children('select').children('option:selected');
        ...
        $('.shiftAllToDisplay').live('click',function(){
            var destinationDiv = $(this).parents('.colControls').siblings('.colDisplayed').children('select');
            var optionsTomove = $(this).parents('.colControls').siblings('.colNotDisplayed').children('select').children('option');
            optionsTomove.each(function(){
                destinationDiv.append($(this));
            });
            return false;
        ...
        $('.shiftToNotDisplay').live('click',function(){
            var destinationDiv = $(this).parents('.colControls').siblings('.colNotDisplayed').children('select');
            var optionsTomove = $(this).parents('.colControls').siblings('.colDisplayed').children('select').children('option:selected');
            optionsTomove.each(function(){
                destinationDiv.append($(this));
            });
            return false;
        ...
        $('.shiftAllToNotDisplay').live('click',function(){
            var destinationDiv = $(this).parents('.colControls').siblings('.colNotDisplayed').children('select');
            var optionsTomove = $(this).parents('.colControls').siblings('.colDisplayed').children('select').children('option');
            optionsTomove.each(function(){
                destinationDiv.append($(this));
            });
            return false;
        ...
        $('#customerRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':customerPerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            customerRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 ...
                 success: function(str){
                     $('.pageOverlay').fadeOut('fast');
                      ...
                      var totalRecords = (endIndex - startIndex) + 1;
                        ...
                        var params = {"searchStr":$('#custSearch input.inputText').val(),
                                "startIndex":startIndex,
                                "endIndex":endIndex,
                                "totalRecords":totalRecords};
        ...
        $('#adjustmentRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':adjustmentPerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            adjustmentRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 url: '/configuration/configure-table-records',
                 data: params,
                 ...
                 success: function(str){
                     $('.pageOverlay').fadeOut('fast');
                      $('#recordsPerPagePopUp').fadeOut('fast');
                      $('.adjustmentPaginationData span.endIndex').html(adjustmentRecordsPerPage);
                     if ($('#adjSearch .inputText').attr('search') == 'on') {
                        ...
                        var totalRecords = (endIndex - startIndex) + 1;
                         var params = {"searchStr":$('#adjSearch input.inputText').val(),
                                "startIndex":startIndex,
                                "endIndex":endIndex,
                                "totalRecords":totalRecords,
                                "sortCol" : sortCol,
        ...
        $('#productRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':productPerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            productRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 url: '/configuration/configure-table-records',
                 data: params,
                 ...
                 success: function(str){
                      ...
                      if($('#prodSearch .inputText').attr('search') == 'on'){
                        var params = {"searchStr":$('#prodSearch input.inputText').val(),
                                "startIndex":startIndex,
                                "endIndex":endIndex,
                                "totalRecords":totalRecords
                        };
        ...
        $('#pricingTypeRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':pricingTypePerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            pricingTypeRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 url: '/configuration/configure-table-records',
                 data: params,
                 ...
                 success: function(str){
                     	$('.pageOverlay').fadeOut('fast');
                     	$('#recordsPerPagePopUp').fadeOut('fast');
                     	$('.pricingTypePaginationData span.endIndex').html(pricingTypeRecordsPerPage); 
        ...
        $('#pricingSeqRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':pricingSeqPerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            pricingSeqRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 url: '/configuration/configure-table-records',
                 data: params,
                 ...
                 success: function(str){
                     	$('.pageOverlay').fadeOut('fast');
                     	$('#recordsPerPagePopUp').fadeOut('fast');
                     	$('.pricingSeqPaginationData span.endIndex').html(pricingSeqRecordsPerPage);                     
        ...
        $('#groupComboRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':groupComboPerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            groupComboRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 url: '/configuration/configure-table-records',
                 data: params,
                 ...
                 success: function(str){
                     	$('.pageOverlay').fadeOut('fast');
                     	$('#recordsPerPagePopUp').fadeOut('fast');
                     	$('.pricingSeqPaginationData span.endIndex').html(groupComboRecordsPerPage);                     
        ...
        $('#prodAttrRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':prodAttrPerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            prodAttrRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 url: '/configuration/configure-table-records',
                 data: params,
                 ...
                 success: function(str){
                     	$('.pageOverlay').fadeOut('fast');
                     	$('#recordsPerPagePopUp').fadeOut('fast');
                     	$('.productAttrPaginationData span.endIndex').html(prodAttrRecordsPerPage);                     
        ...
        $('#custAttrRecordsPerPage').live('click',function(){
            ...
            var params = {'accessname':custAttrPerPageRecords,
                    'val':$('#recordPerPageForm input#records').val()};
            custAttrRecordsPerPage = parseInt($('#recordPerPageForm input#records').val());
            jQuery.ajax({
                 type: 'POST',
                 url: '/configuration/configure-table-records',
                 data: params,
                 ...
                 success: function(str){
                     	$('.pageOverlay').fadeOut('fast');
                     	$('#recordsPerPagePopUp').fadeOut('fast');
                     	$('.customerAttrPaginationData span.endIndex').html(custAttrRecordsPerPage);                     
        ...
        $('#pricingSeqRecordsTable .expandRowLink')
        .live(
                'click',
                function() {
                    $(this).addClass('hidden');
                    $(this).siblings('.collapseRowLink').removeClass(
                            'hidden');
                    var rowId = $(this).parents('tr.expandableRow').attr(
                            'id').split("_");
                    var pricingTypeRow = $('<tr>').addClass(
                            'editPricingSeqRowWrapper');
                    var pricingTypeTd = $('<td>').attr('colspan', '3')
                            .addClass('editPricingSeqRow');
                    var divEle = $('<div>').addClass(
                            'pricingSeqTableWrapper');
                    var tblEle = $('<table>').addClass(
                            'recordsTable pricingSeqDetailsTable');
                    var tblHead = $("<thead>");
                    var tblHeadRow = $("<tr>");
                    var pricingTypeCol = $("<th>").addClass('priceTypeCol')
                            .append('Pricing Type');
                    var targetCol = $("<th>").addClass('targetCol').append(
                            'Target');
                    var extraTh = $('<th>');
                    var recordTbl = $("<table>")
                            .addClass(
                    ...
                    $(this).parents('tr.expandableRow').addClass(
                            'expandedRow');
                    $(this).parents('tr.expandableRow').after(
                            pricingTypeRow);
                    ...
                    $("table.updatePricingTypeNewSeqTable tbody")
                            .droppable(
                                    {
        ...
        $('#editPricingSequence').live('click',function(){
            ...
            if(selPricingTypeIds.indexOf(",")>=0){
                selPricingTypeIds=selPricingTypeIds.substr(0,selPricingTypeIds.length-1);
            }
        ...
        $('#pricingSeqRecordsTable .collapseRowLink').live('click',function(){
            ...
            $(this).parents('tr.expandableRow').next('.editPricingSeqRowWrapper').remove();
            return false;
        });

        $('#pricingSeqRecordsTable .editPricingSeqPricingTypeRow').live('click',function(){
            $(this).parent('td').css({'width':'65px'});
            $(this).siblings('.savePricingLink, .closeEditRowLink').toggle();
            if($(this).parents('tr').next('.editPricingSeqRowWrapper').is(":visible") == false){
                $(this).parents('td').siblings('td').children('.expandRowLink').trigger('click');
            }
            var rowId=$(this).parents('tr.expandableRow').attr('id').split("_");
            var seq_description=$(this).parents('td').siblings('.pricing-seq-desc').text();
            $(this).parents('td').siblings('.pricing-seq-desc').text('');
            var seq_des_txt=$('<input>').addClass("inputText pricing-seq-desc-txt_"+rowId[1]).attr("type", "text").attr("value",seq_description);
            $(this).parents('td').siblings('.pricing-seq-desc').append(seq_des_txt);

            $('.editPricingSeqRowWrapper .deleteSeqEditRow').show();
            $(this).parents('td').siblings('.pricingSeqName').children('.inputText').removeClass('readonlyInput').removeAttr('readonly').focus();
            $(this).hide();
            return false;
        });
        $('#pricingSeqRecordsTable .closeEditRowLink').live('click',function(){
            $(this).parent('td').css({'width':'48px'});
            $(this).siblings('.savePricingLink, .editPricingSeqPricingTypeRow').toggle();
            $(this).parents('td').siblings('td').children('.collapseRowLink').trigger('click');
            $(this).parents('td').siblings('.pricingSeqName').children('.inputText').addClass('readonlyInput').attr('readonly');
            var rowId=$(this).parents('tr.expandableRow').attr('id').split("_");
            var seq_description=$(this).parents('td').siblings('.pricing-seq-desc').children('.pricing-seq-desc-txt_'+rowId[1]).val();
        ...
        .live(
                ...
                function() {
                    ...
                    var parentRowhtml = "<tr class='expandableRow expandedRow' id='"+rowId+"'><td class='first-child'>"
                    ...
                    var editRowHtml = "<tr class='editPricingSeqRowWrapper'><td colspan='3' class='editPricingSeqRow'><div class='pricingSeqTableWrapper'><table class='recordsTable pricingSeqDetailsTable'><thead><tr><th class='priceTypeCol'>Pricing Type</th>"
                    ...
                    $('#pricingSeqRecordsTable').append(parentRowhtml)
                            .append(editRowHtml);

                    $('.deleteNewPricingSeqOpened').live(
                            'click',
                            function() {
                                $(this).parents('.expandableRow').next(
                                        '.editPricingSeqRowWrapper')
                                        .remove();
                    ...
                    $("table.addPricingTypeNewSeqTable tbody")
                            .droppable(
                                    {
                                        // activeClass: "ui-state-default",
                                        // hoverClass: "ui-state-hover",
...
function dropOperationPricingTypeSeq(selTr, ui, rowId) {
    ...
    pricingTypeRowNew.attr("id", 'pricingTypeId_'
            + ui.draggable.children('.first-child').text() + "_" + rowId);    
    pricingTypeRowNew.addClass('pricingSeqOperation');
    var pricingTypeCol = $('<td>').addClass('pricingTypeCol').append(
            ui.draggable.children('.pricing-type').text());
    ...
    var pricingTypeIdHid = $('<input>').addClass(
            "pricingTypeIdHid_"+rowId).attr("type", "hidden").attr(
            "value", ui.draggable.children('.first-child').text());
    ...
    pricingTypeRowNew.prependTo(selTr);
        ...
        function loadUIAdjustmentData(adjustment) {
            var unid = typeof(adjustment["unid"]) != 'undefined' ? adjustment["unid"] : "";
            var objectType = typeof(adjustment["objectType"]) != 'undefined' ? adjustment["objectType"] : "";
            var objectName = typeof(adjustment["objectName"]) != 'undefined' ? adjustment["objectName"] : "";
            var organizationId = typeof(adjustment["organizationId"]) != 'undefined' ? adjustment["organizationId"] : "";
            var organizationName = typeof(adjustment["organizationName"]) != 'undefined' ? adjustment["organizationName"] : "";
            var prcTypeDesc = typeof(adjustment["prcTypeDesc"]) != 'undefined' ? adjustment["prcTypeDesc"] : "";
            var amount = typeof(adjustment["amount"]) != 'undefined' ? adjustment["amount"] : "";
            display='visible';
            ...
            var unidEle = $('<td style="display:'+display+'">').addClass('first-child').append(unid);
            ...
            var objectTypeEle = $('<td style="display:'+display+'">').append(objectType);
            if($('#ADJ_OBJECT_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var objectNameEle = $('<td style="display:'+display+'">').append(objectName);
            if($('#ADJ_ORG_ID').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var orgidEle = $('<td style="display:'+display+'">').append(organizationId);
            if($('#ADJ_ORG_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var orgNameEle = $('<td style="display:'+display+'">').append(organizationName);
            if($('#ADJ_PRICING_TYPE').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var prcTypeDescEle = $('<td style="display:'+display+'">').append(prcTypeDesc);
            ...
            var editEleContent = $('<a>').attr('href', "").addClass("editRowLink").append(editImage);
            var deleteEleContent = $('<a>').attr('href', "").addClass("deleteRowLink").append(deleteImage);
            var lastEle = $('<td>').addClass('last-child').append(editEleContent).append(deleteEleContent);
            body.append(unidEle);
        ...
        function loadHiddenAdjustment(adjustment) {
            ...
            var organizationId = typeof(adjustment["organizationId"]) != 'undefined' ? adjustment["organizationId"] : "";
            var organizationName = typeof(adjustment["organizationName"]) != 'undefined' ? adjustment["organizationName"] : "";
            var pricingTypeId = typeof(adjustment["pricingTypeId"]) != 'undefined' ? adjustment["pricingTypeId"] : "";
            var amount = typeof(adjustment["amount"]) != 'undefined' ? adjustment["amount"] : "";
            var rangeType = typeof(adjustment["rangeType"]) != 'undefined' ? adjustment["rangeType"] : "";
            var lowRange = typeof(adjustment["lowRange"]) != 'undefined' ? adjustment["lowRange"] : "";
            var highRange = typeof(adjustment["highRange"]) != 'undefined' ? adjustment["highRange"] : "";
            var commentText = typeof(adjustment["commentText"]) != 'undefined' ? adjustment["commentText"] : "";
            var startDate = typeof(adjustment["startDate"]) != 'undefined' ? adjustment["startDate"] : "";
            var endDate = typeof(adjustment["endDate"]) != 'undefined' ? adjustment["endDate"] : "";
            var customerName = typeof(adjustment["customerName"]) != 'undefined' ? adjustment["customerName"] : "";
            var customerId = typeof(adjustment["customerId"]) != 'undefined' ? adjustment["customerId"] : "";
            var objectId = typeof(adjustment["objectNameId"]) != 'undefined' ? adjustment["objectNameId"] : "";
            ...
            var objectTypeEle = $('<span>').addClass("adjObjectType").append(objectType);
            var adjOrgIdsEle = $('<span>').addClass("adjOrgIds").append(organizationId);
            var pricingTypeIdEle = $('<span>').addClass("adjPricingTypeId").append(pricingTypeId);
            ...
            var commentTextEle = $('<span>').addClass("adjCommentText").append(commentText);
            var custNameEle = $('<span>').addClass("adjCustName").append(customerName);
            var custIdEle = $('<span>').addClass("adjCustId").append(customerId);
            var orgnizationNameEle = $('<span>').addClass("adjOrgs ").append(organizationName);
            var objectNameEle = $('<span>').addClass("adjProdGroups").append(objectName);
            var objectIdEle = $('<span>').addClass("adjProdGroupIds").append(objectId);

            body.append(custNameEle).append(custIdEle).append(objectNameEle).append(objectIdEle);
            body.append(startDateEle).append(endDateEle).append(unidEle).append(amountEle);
            body.append(objectTypeEle).append(orgnizationNameEle).append(pricingTypeIdEle).append(adjOrgIdsEle);
            body.append(rangeTypeEle).append(lowRangeEle).append(highRangeEle).append(commentTextEle);
            $('#adjustmentDetails').append(body);
        }

        function loadUIProductData(product) {
            var unid = typeof(product["unid"]) != 'undefined' ? product["unid"] : "";
            var customerName = typeof(product["customerName"]) != 'undefined' ? product["customerName"] : "";
            var endDate = typeof(product["endDate"]) != 'undefined' ? product["endDate"] : "";
            var organizationId = typeof(product["organizationId"]) != 'undefined' ? product["organizationId"] : "";
            var organizationName = typeof(product["organizationName"]) != 'undefined' ? product["organizationName"] : "";
            var masterName = typeof(product["masterName"]) != 'undefined' ? product["masterName"] : "";
            var productGroupName = typeof(product["productGroupName"]) != 'undefined' ? product["productGroupName"] : "";
            var startDate = typeof(product["startDate"]) != 'undefined' ? product["startDate"] : "";
            display='visible';
            ...
            var orgidEle = $('<td style="display:'+display+'">').addClass('first-child org-id').append(organizationId);
            if($('#PROD_ORG_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var orgNameEle = $('<td style="display:'+display+'">').addClass('product-name').append(organizationName);
            if($('#PROD_CUSTOMER_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var custNameEle = $('<td style="display:'+display+'">').append(customerName);
            if($('#PROD_MASTER_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var masterNameEle = $('<td style="display:'+display+'">').append(masterName);
            if($('#PROD_GROUP_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var prodGrpNameEle = $('<td style="display:'+display+'">').append(productGroupName);
            var editImage = $('<img>').attr('src', 'resources/images/editRow.png');
            var deleteImage = $('<img>').attr('src', 'resources/images/deleteRow.png');
            var editEleContent = $('<a>').attr('href', "").addClass("editRowLink").append(editImage);
            var deleteEleContent = $('<a>').attr('href', "").addClass("deleteRowLink").append(deleteImage);
        ...
        function loadProductHiddenData(product) {
            ...
            var masterName = typeof(product["masterName"]) != 'undefined' ? product["masterName"] : "";
            var productGroupName = typeof(product["productGroupName"]) != 'undefined' ? product["productGroupName"] : "";
            var productGroupId = typeof(product["productGroupId"]) != 'undefined' ? product["productGroupId"] : "";
            var commentText = typeof(product["commentText"]) != 'undefined' ? product["commentText"] : "";
            ...
            var masterNameEle = $('<span>').addClass("prodMasterPartName").append(masterName);
            var custNameEle = $('<span>').addClass("prodCustomerPartName").append(customerName);
            var groupNameEle = $('<span>').addClass("productGroups").append(productGroupName);
            var groupIdEle = $('<span>').addClass("prodGroupId").append(productGroupId);
            var orgIdEle = $('<span>').addClass("prodOrgId").append(organizationId);
            var orgNameEle = $('<span>').addClass("prodOrgName").append(organizationName);

            var commentEle = $('<span>').addClass("prodComment").append(commentText);
            var extraCol = product.extraColumn;
            body.append(startDateEle).append(endDateEle).append(unidEle).append(masterNameEle).append(custNameEle);
            body.append(groupNameEle).append(orgIdEle).append(orgNameEle).append(commentEle).append(groupIdEle);
        ...
        function loadCustomerHiddenData(customer) {
            var endDate = typeof(customer["endDate"]) != 'undefined' ? customer["endDate"] : "";
            var groupId = typeof(customer["groupId"]) != 'undefined' ? customer["groupId"] : "";
            var name = typeof(customer["name"]) != 'undefined' ? customer["name"] : "";
            var startDate = typeof(customer["startDate"]) != 'undefined' ? customer["startDate"] : "";
            var unid = typeof(customer["unid"]) != 'undefined' ? customer["unid"] : "";
            var groupName = typeof(customer["groupName"]) != 'undefined' ? customer["groupName"] : "";
            var id = typeof(customer["id"]) != 'undefined' ? customer["id"] : "";
            var commentText = typeof(customer["commentText"]) != 'undefined' ? customer["commentText"] : "";
            ...
            body.append(startDateEle).append(endDateEle).append(unidEle).append(groupIdEle);
        ...
        function loadUICustomerData(customer) {
            ...
            var unidEle = $('<td style="display:'+display+'">').addClass('first-child customer-id customerTableDataLayout').attr('unid','').append(unid);
            if($('#CUST_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var nameEle = $('<td style="display:'+display+'">').addClass('customer-name customerTableDataLayout').attr('unid','').append(name);
            if($('#CUST_GROUP_ID').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var groupIdEle = $('<td style="display:'+display+'">').addClass('customerTableDataLayout').attr('unid','').append(groupId);            
            if($('#CUST_GROUP_NAME').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var groupNameEle = $('<td style="display:'+display+'">').addClass('customerTableDataLayout customerOrganizationName').attr('unid','').append(groupName);           
            if($('#CUST_START_DATE').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var startDateEle = $('<td style="display:'+display+'">').addClass('customerTableDataLayout').attr('unid','').append(startDate);           
            if($('#CUST_END_DATE').is(':visible')==true){
            	display='visible';
            }else{
            	display='none';
            }
            var endDateEle = $('<td style="display:'+display+'">').addClass('customerTableDataLayout').attr('unid','').append(endDate);
            var editImage = $('<img>').attr('src', 'resources/images/editRow.png');
            var deleteImage = $('<img>').attr('src', 'resources/images/deleteRow.png');
            var editEleContent = $('<a>').attr('href', "").addClass("editRowLink").append(editImage);
            var deleteEleContent = $('<a>').attr('href', "").addClass("deleteRowLink").append(deleteImage);
            var lastEle = $('<td>').addClass('last-child').append(editEleContent).append(deleteEleContent);
            body.append(unidEle).append(nameEle).append(groupIdEle).append(groupNameEle);
        ...
        function loadFilters(params,divFor){
            ...
            $.each(params,function(iterator,param){
                if(iterator < 5){
                var list = $('<li>');
                var name=$('<a>').attr({href:"",style:"white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"}).addClass("filterLink "+divFor+"FilterName").append(param.name);
                var id=$('<span>').attr('style',"display: none").addClass(divFor+"FilterId").append(param.id);
                var reqData=$('<div>').attr('style',"display: none").addClass(divFor+"FilterReqData hoverData").append(param.reqData);
                ...
                var seeMoreLink = $('<a>').attr('href',"").addClass('seeMoreLink').append('See More');
                var uList=$('<ul>').addClass('allFiltersList '+divFor+'AllFilterList');
        ...
        function loadCustAttributeData(attribute){
            var columnName = typeof(attribute["columnName"]) != 'undefined' ? attribute["columnName"] : "";
            var type = typeof(attribute["columnType"]) != 'undefined' ? attribute["columnType"] : "";
            var description = typeof(attribute["description"]) != 'undefined' ? attribute["description"] : "";
            ...
            var custDescription = $('<td>').addClass('customerAttributeName').append(description);
            var custColumnName = $('<td>').append(columnName);
            var custType = $('<td>').append(type);
            var deleteImage = $('<img>').attr('src', 'resources/images/deleteRow.png');
            var deleteContent = $('<a>').attr('href', "").addClass("deleteRowLink deleteCustomerAttribute").append(deleteImage);
            var lastElement = $('<td>').addClass('last-child').append(deleteContent);
            body.append(custUnid).append(custDescription).append(custColumnName).append(custType).append(lastElement);
        ...
        function loadProAttributeData(attribute){
            ...
            var custDescription = $('<td>').addClass('productAttributeName').append(description);
            var custColumnName = $('<td>').append(columnName);
            var custType = $('<td>').append(type);
            var deleteImage = $('<img>').attr('src', 'resources/images/deleteRow.png');
            var deleteContent = $('<a>').attr('href', "").addClass("deleteRowLink deleteProductAttribute").append(deleteImage);
            var lastElement = $('<td>').addClass('last-child').append(deleteContent);
            body.append(custUnid).append(custDescription).append(custColumnName).append(custType).append(lastElement);
        ...
        if($('input#attributeNumeric').is(':checked') == true) {
           ...
           type = '5';
            ...
            var params = {"description" :$('#customerAttributeForm input#customerColumnDescription').val(),
        ...
        $('#addProductAttribute').live('click',function(){
            ...
            var params = {"description" :$('#createProductAttributeForm input#productColumnDescription').val(),
                      "columnName" : $('#createProductAttributeForm input#productColumnName').val(),
                      "type" : type
                     };
        ...
        $('#productsRecordTable .next').live('click', function(){
            ...
            var params = {"startIndex": startIndex, "endIndex" : endIndex, "sortCol" : sortCol, "sortDir" : sortDir};
        ...
        function loadCustomerData(params) {
            ...
            jQuery.ajax({
                ...
                success: function(str){
                    ...
                    $('#totalCustomers').html('Customers - showing '+str.customers.listSize+' of ' +str.customers.totalRecords);
                    var totalPages = str.customers.totalRecords/str.customers.tableRecords;
        ...
        function loadAdjustmentData(params) {
            ...
            jQuery.ajax({
                ...
                success: function(str){
                    ...
                    $('#totalAdjustments').html("Adjustments - showing "+str.adjustments.listSize+' of '+str.adjustments.totalRecords);
                    var totalPages = str.adjustments.totalRecords/adjustmentRecordsPerPage;
        ...
        function loadProductData(params) {
            ...
            jQuery.ajax({
                ...
                success: function(str){
                    ...
                    $('#totalProducts').html("Products - showing "+str.products.listSize+' of '+str.products.totalRecords);
        ...
        function loadCustomerAttributesData(params){
            ...
            jQuery.ajax({
                ...
                dataType: 'json',
                success: function(str){
                    $('#totalCustomerAttributes').html('Customer Attributes - showing '+str.customerAttributes.listSize+' of '+str.customerAttributes.totalRecords);
                    var totalPages = str.customerAttributes.totalRecords/custAttrRecordsPerPage;
                    $('#CustomersAttributesTable span.totalPage').text(Math.ceil(totalPages));
                    loadAllCustomerAttributeData(str.customerAttributes.attributes);
                    $('.customerAttrPaginationData span.startIndex').text(params.startIndex);
        ...
        function loadProductAttributeData(params){
            ...
            jQuery.ajax({
                ...
                dataType: 'json',
                success: function(str){
                    $('#totalProductAttributes').html('Product Attributes - showing '+str.productAttributes.listSize+' of '+str.productAttributes.totalRecords);
                    var totalPages = str.productAttributes.totalRecords/prodAttrRecordsPerPage;
        ...
        $('#adjSearch .serachButton').live('click', function(){
            ...
            $('.adjustmentTableFiltersDisplayList').siblings().children('.italicText').text('');
        	if($('#adjSearch input.inputText').val() == 'Enter Object Name to Search'){
                return false;
            }
            $('#adjSearch .inputText').attr('search','on');
            var startIndex = parseInt($('.adjustmentPaginationData span.startIndex').text());
            var endIndex = parseInt($('.adjustmentPaginationData span.endIndex').text());
            var totalRecords = (endIndex - startIndex) + 1;
            var params = {"searchStr":$('#adjSearch input.inputText').val(),
                    "startIndex":startIndex,
                    "endIndex":endIndex,
        ...
        $('#prodSearch .serachButton').live('click', function(){
        	...
        	$('.productsTableFiltersDisplayList').siblings().children('.italicText').text('');
        	if($('#prodSearch input.inputText').val() == 'Enter CustomerPartName/MasterPartName to Search'){
                return false;
            }
            $('#prodSearch .inputText').attr('search','on');
            var startIndex = parseInt($('.productPaginationData span.startIndex').text());
        ...
        $('#savePricingSequence').live('click',function(){
            ...
            var hidPricingId=$('.pricingTypeIdHid_'+$(this).parents('tr').attr('id'));
            var selPricingTypeIds="";
            hidPricingId.each(function(i){
                selPricingTypeIds+=$(this).val()+",";
            });           
            ...
            var params= {
                    "description":$(this).parents('td').siblings('.pricingSeqName').children('.inputText').val(),
                    "unid": $(this).parents('td').siblings('.first-child').children('.inputText').val(),
                    "pricingTypeIds": selPricingTypeIds
            };
            jQuery.ajax({
                    type: 'POST',
        ...
        $('#prodSearch a.genLink').live('click', function(){
            ...
            $('.productsTableFiltersDisplayList').empty();
            $('.productsTableFiltersDisplayList').siblings().children('.italicText').text('');
            $('#prodSearch input').val("Enter CustomerPartName/MasterPartName to Search");
            getProductFirstPage(false);
            return false;
        });

        $('#customerRecordsTableDeletePopup input.delCustomer').live('click', function(){
            var params = {"unid" : $('#customerRecordsTableDeletePopup span#delete-cust').text()};
        ...
        $('#adjustmentsRecordsTableDeletePopup input.delAdjustment').live('click', function(){
            var params = {"unid" : $('#adjustmentsRecordsTableDeletePopup span#delete-adj').text()};
        ...
        $('#productsRecordsTableDeletePopup input.delProduct').live('click', function(){
            var params = {"unid" : $('#productsRecordsTableDeletePopup span#delete-prod').text()};
        ...
        $('#productUpdate').live('click', function(){
            ...
            var params = {"unid": $('#editProduct td.prodUnid').text(),
       ...
       jQuery.ajax({
        ...
        $('#customerUpdate').live('click', function(){
            ...
            var params = {"custUniqueId": $('#editCustomer input.custUnid').val(),
        ...
        $('#hierarchyUpdate').live('click',function(){
            ...
            jQuery.ajax({
                ...
                success: function(str){
                    ...
                    $('.popupWrapper').fadeOut('fast');
                    var data= $('.jstree-clicked').parents('.hierarchyContainer').attr('id');
                    if(data == 'organizationContainer' && $('.hierarachy-Div>ul>li>a').hasClass('jstree-clicked')){
                        $.jstree._reference($("#organizationHierarchy")).refresh(-1);
                    }
                    if(data == 'productContainer' && $('.hierarachy-Div>ul>li>a').hasClass('jstree-clicked')){
                        $.jstree._reference($("#productHierarchy")).refresh(-1);
                    }
                    if(data == 'organizationContainer' && $('.hierarachy-Div>ul>li>a').hasClass('jstree-clicked') == false){
                         $.jstree._reference($("#organizationHierarchy")).refresh($('li[unid="'+$('.jstree-clicked').parents().attr('parentId')+'"]'));
                     } else{
                         $.jstree._reference($("#productHierarchy")).refresh($('li[unid="'+$('.jstree-clicked').parents().attr('parentId')+'"]'));
                    }
                },
            error: function(XMLHttpRequest, textStatus,errorThrown){
                // TODO: need to show error message
        ...
        $('#updateAdjustment').live('click', function(){
            ...
            var params = {"unid" : $('#editAdjustment td.adjUnid').text(),
       ...
       jQuery.ajax({
        ...
        $('#addCustomer').live('click', function(){
            if(validation('createCustomer') && validateNumeric('createCustomer')){
                var params = {"custUniqueId": $('#createCustomer input#custUniqueId').val(),
        ...
        $('.customerFilterName').live('click',function(){
            ...
            var endIndex = parseInt($('.customerPaginationData span.endIndex').text());
            var totalRecords = (endIndex - startIndex) + 1;
            var params={'filterId':$(this).siblings('.customerFilterId').html(),
        ...
        $('.adjustmentFilterName').live('click',function(){
            ...
            var endIndex = parseInt($('.adjustmentPaginationData span.endIndex').text());
            var totalRecords = (endIndex - startIndex) + 1;
            var params={'filterId':$(this).siblings('.adjustmentFilterId').html(),
                        'filterName':$(this).html(),
                        'startIndex':startIndex,
                        'endIndex':endIndex,
                        'totalRecords':totalRecords
        ...
        $('.productFilterName').live('click',function(){
            ...
            var endIndex = parseInt($('.productPaginationData span.endIndex').text());
            var totalRecords = (endIndex - startIndex) + 1;
            var params={'filterId':$(this).siblings('.productFilterId').html(),
                        'filterName':$(this).html()
                        };
            loadProductAdvanceSearchData(params);
        ...
        $('#addGroupCombo').live('click',function(){
            if(validation('createGroupComboForm') && $.isNumeric($('#createGroupComboForm input#groupPriority').val())){
            var type = '1';
            if($('#createGroupComboForm input#groupOrganization').is(':checked') == true) {
                type = '1';
            }else if ($('#createGroupComboForm input#groupProduct').is(':checked') == true){
               type = '2';
            }
            var groupId=[];
            var modifier=[];
            $('#createGroupComboForm td.organizationId').each(function(){
                ...
                $("#"+$(this).siblings().children().attr('id')+" option:selected").each(function(){
        ...
        $('#groupComboEdit').live('click',function(){
            ...
            var type = '1';
            if($('#editGroupComboForm input#groupComboEditOrganization').is(':checked') == true) {
                type = '1';
            }else{
               type = '2';
            }
            ...
            $('#editGroupComboForm td.organizationId').each(function(){
                groupId.push($(this).parent().attr('id'));
                modifier.push($('#editGroupComboForm select#groupComboModifier'+$(this).parent().attr('id')+' option:selected').val());
            ...
            var params = {
                          ...
                          'generic': $('#editGroupComboForm input#addGenCustProduct').is(':checked') ? 'true' : 'false'
                          };
            jQuery.ajax({
                 type: 'POST',
                 url: url,
        ...
        $('#addSiblingHir').live('click', function(){
           ...
           var params = {"unid": $('#addSibling input#siblingUnid').val(),
        ...
        $('#addChildHir').live('click', function(){
            if(validation('addChildGroup') == true){
             var params = {"unid": $('#addChild input#unid').val(),
             ...
             jQuery.ajax({
                 ...
                 success: function(str){
                     ...
                     var data= $('.jstree-clicked').parents('.hierarchyContainer').attr('id');
                     if(data == 'organizationContainer'){
                         $.jstree._reference($("#organizationHierarchy")).refresh($('.jstree-clicked'));
                     } else{
                         $.jstree._reference($("#productHierarchy")).refresh($('.jstree-clicked'));
                     }
                 },
                 error:function(XMLHttpRequest, textStatus,errorThrown){
                     // TODO: need to show error message form the Server
        ...
        $('#addAdjustment').live('click', function(){
            if(validation('createAdjustment')){
                var params = {"amount": $('#createAdjustment input.adjAmount').val(),
        ...
        $('#addProduct').live('click', function(){
            if(validation('createProduct')){
                var params = {"custPartName": $('#createProduct input#custPartName').val(),
    ...
    $('#advAdjustmentSearchBtn').live('click', function(){
            ...
            var params = {
                    ...
                    "genIdMatch": $('#adjustFilterGenIdMatch').is(':checked') ? 'true' : 'false',
                    "genEffDateMatch": $('#adjustFilterGenEffDateMatch').is(':checked') ? 'true' : 'false',
                    ...
                    "matchAllFields": $('#adjustmentNewFilterAllMatchCheck').is(':checked') ? 'true' : 'false'
                };
    ...
    function loadAdjustmentAdvanceSearch(params){
    	...
    	jQuery.ajax({
            ...
            success: function(str){
            	...
            	$('.adjustmentTableFiltersDisplayList').empty();
                $('.adjustmentTableFiltersDisplayList').siblings().children('.italicText').text('filtered by');
                var filterMarkup = '<li><a href="#" class="removeFilterLink"><img src="resources/images/removeFilter.png"></a><span>'+str.adjustments.filterName+'</span></li>';
    ...
    $('#proSrcCustId').live('blur', function(){
        $('#advSearchCustName').html('');
        if(jQuery.isEmptyObject($('#prodAdvanceSearch input#proSrcCustId').val())){
            return;
        }
    ...
    $('#adjSerPrcType').live('change',function(){
        ...
        if(index!=0){
            ...
            $('#advSearchPrcOperation').html($('#pricingTypeOperationDropDown').find(":selected").text());
        }else{
    ...
    $('#advCustomerSearchBtn').live('click', function(){
            ...
            var params = {
                    ...
                    "custUnidMatch": $('#custAdvSrcGenIdMatch').is(':checked') ? 'true' : 'false',
                    "custToDate": $('#custAdvanceSearch input#custAdvSrcToDate').val(),
                    "custFromDate": $('#custAdvanceSearch input#custAdvSrcFromDate').val(),
                    "custAllMatchCheck": $('#customerNewFilterAllMatchCheck').is(':checked') ? 'true' : 'false',
                    "custEffDateMatch":    $('#custAdvSrcEffDateMatch').is(':checked') ? 'true' : 'false',
                    "sortCol": 'unid',
                    "sortDir": 'asc',
                    "startIndex":startIndex,
                    "endIndex":endIndex,
    ...
    function loadCustomerAdvanceSearchData(params) {
    	...
    	jQuery.ajax({
            ...
            success: function(str){
                ...
                $('.customerTableFiltersDisplayList').empty();
                $('.customerTableFiltersDisplayList').siblings().children('.italicText').text('filtered by');
                var filterMarkup = '<li><a href="#" class="removeFilterLink"><img src="resources/images/removeFilter.png"></a><span>'+str.customersAdvRes.filterName+'</span></li>';
                $('.customerTableFiltersDisplayList').append(filterMarkup);
                $('#totalCustomers').html("Customers - showing "+str.customersAdvRes.customerList.length+' of '+str.customersAdvRes.totalRecords);
                var totalPages = str.customersAdvRes.totalRecords/customerRecordsPerPage;
                $('#customerRecordTable span.totalPage').text(Math.ceil(totalPages));
                hidePagination('customerTablePagination');
    ...
    $('#advProductSearchBtn').live('click',function(){
        ...
        var params={
                ...
                "customerPartNameMatch": $('#proSrcCustomerPartNameMatch').is(':checked') ? 'true' : 'false',
                "mastName": $('#prodAdvanceSearch input#proSrcMastName').val(),
                "masterPartNameMatch": $('#proSrcMasterPartNameMatch').is(':checked') ? 'true' : 'false',
                "effDateFrom": $('#prodAdvanceSearch input#proSrcEffDateFrom').val(),
                "effDateTo": $('#prodAdvanceSearch input#proSrcEffDateTo').val(),
                "effDateMatch": $('#productFilterEffDateMatch').is(':checked') ? 'true' : 'false',
                "allMatchCheck": $('#productNewFilterAllMatchCheck').is(':checked') ? 'true' : 'false'
        };
        loadProductAdvanceSearchData(params);
        $('.pageOverlay').fadeOut('fast');
        $('.popupWrapper').fadeOut('fast');
    ...
    function loadProductAdvanceSearchData(params){
    	...
    	jQuery.ajax({
            ...
            success: function(str){
            	...
            	$('.productsTableFiltersDisplayList').empty();
                $('.productsTableFiltersDisplayList').siblings().children('.italicText').text('filtered by');
                var filterMarkup = '<li><a href="#" class="removeFilterLink"><img src="resources/images/removeFilter.png"></a><span>'+str.productAdvRes.filterName+'</span></li>';
                $('.productsTableFiltersDisplayList').append(filterMarkup);
                $('#totalProducts').html("Products - showing "+str.productAdvRes.listSize+' of '+str.productAdvRes.totalRecords);
        ...
        jQuery.ajax({
            ...
            success: function(str){
                $.each(str.pricingOperations,function(){
                    ...
                    var description=typeof(this["description"]) != 'undefined' ? this['description'] : "";
    ...
    $("#createPricingTypeBtn").live("click",function(){
        if(validation('createPricingTypeForm') && pricingOperationValidation('#pricingTypeOperationDropDown')){
            ...
            var params={
                    ...
                    "optionalArgs": $('#createPricingTypeForm textarea#pricingTypeOptArgs').val()
            };
            jQuery.ajax({
                type: 'POST',
                url: url,
    ...
    $("#editPricingTypeBtn").live("click",function(){
        ...
        if(validation('editPricingTypeForm') && pricingOperationValidation('#pricingTypeOperationDropDownEdit')){
            ...
            var params={
                    ...
                    "operationId": $('#pricingTypeOperationDropDownEdit').find(':selected').val(),
                    "targetId": $('#targetTypeDropDownEdit').find(':selected').val(),
                    "optionalArgs": $('#editPricingTypeForm textarea#pricingTypeOptArgs').val()
            };
            jQuery.ajax({
                type: 'POST',
                url: url,
    ...
    function loadPricingTypesData(params) {
        ...
        jQuery.ajax({
            ...
            success: function(str){
                //display UI data and hidden data
                $('#totalPricingTypes').html('Pricing Types - showing '+str.pricingTypes.listSize+' of '+str.pricingTypes.totalPricingTypeRecords);
                var totalPages = str.pricingTypes.totalPricingTypeRecords/pricingTypeRecordsPerPage;
    ...
    function loadUIPricingTypeData(pricingType) {
        var unid = typeof(pricingType["unid"]) != 'undefined' ? pricingType["unid"] : "";
        var description = typeof(pricingType["description"]) != 'undefined' ? pricingType["description"] : "";
        var operation = typeof(pricingType["operation"]) != 'undefined' ? pricingType["operation"] : "";
        var target = typeof(pricingType["targetText"]) != 'undefined' ? pricingType["targetText"] : "";
        var optionalArgs = typeof(pricingType["optionalArgs"]) != 'undefined' ? pricingType["optionalArgs"] : "";
        var body = $('<tr>').attr("id", "pritype_"+unid);
        var unidEle = $('<td>').addClass('first-child priTypeUnid').append(unid);
        var descriptionEle = $('<td>').addClass('pricing-type pricingTypeDescription').append(description);
        ...
        var deleteImage = $('<img>').attr('src', 'resources/images/deleteRow.png');
        var editEleContent = $('<a>').attr('href', "").addClass("editRowLink").append(editImage);
        var deleteEleContent = $('<a>').attr('href', "").addClass("deleteRowLink deletePricingType").append(deleteImage);
        var lastEle = $('<td>').addClass('last-child').append(editEleContent).append(deleteEleContent);
        body.append(unidEle).append(descriptionEle).append(operationEle).append(targetEle);
    ...
    function loadHiddenPricingTypeData(pricingType) {
        var unid = typeof(pricingType["unid"]) != 'undefined' ? pricingType["unid"] : "";
        var description = typeof(pricingType["description"]) != 'undefined' ? pricingType["description"] : "";
        var operationId = typeof(pricingType["operation"]) != 'undefined' ? pricingType["operation"] : "";
        var target = typeof(pricingType["targetText"]) != 'undefined' ? pricingType["targetText"] : "";
        var optionalArgs = typeof(pricingType["optionalArgs"]) != 'undefined' ? pricingType["optionalArgs"] : "";
        ...
        var descriptionEle = $('<span>').addClass('priTypeDescription').append(description);
        var operationEle = $('<span>').addClass('priTypeOperationId').append(operationId);
        var targetEle = $('<span>').addClass('priTypeTarget').append(target);
        var optArgsEle = $('<span>').addClass('priTypeOptionalArgs').append(optionalArgs);
        body.append(unid).append(unidEle).append(descriptionEle).append(operationEle).append(targetEle).append(optArgsEle);
    ...
    function loadPricingSequenceData(params) {
        ...
        jQuery.ajax({
            ...
            success: function(str){
                //display UI data and hidden data
                $('#totalPricingSequences').html('Pricing Sequences - showing '+str.pricingSequences.listSize+' of '+str.pricingSequences.totalPricingSeqRecords);
                var totalPages = str.pricingSequences.totalPricingSeqRecords/pricingSeqRecordsPerPage;
    ...
    function loadUIPricingSequenceData(pricingType) {
        var unid = typeof(pricingType["unid"]) != 'undefined' ? pricingType["unid"] : "";
        var description = typeof(pricingType["description"]) != 'undefined' ? pricingType["description"] : "";
        var body = $('<tr>').attr("id", "pritype_"+unid).addClass('expandableRow');
        var unidEle = $('<td>').addClass('first-child');
        var expandImg=$('<img>').attr('src','resources/images/expand-row.png');
        var expandRowLinkEle=$('<a>').attr('href',"").addClass('expandRowLink').append(expandImg);
        var collapseImg=$('<img>').attr('src','resources/images/collapse-row.png');
        var collapseRowLinkEle=$('<a>').attr('href',"").addClass('collapseRowLink hidden').append(collapseImg);
        var unidSpan=$('<span>').attr('id','sequenceUnid').append(unid);

        var descriptionEle=$('<td>').addClass('pricing-seq-desc').append(description);

        var lastChild=$('<td>').addClass('last-child');

        var saveLinkImg=$('<img>').attr('src','resources/images/saveLink.png');
        var savePriLink=$('<a>').attr('href',"#").addClass('savePricingLink hidden');
        savePriLink.attr('id','editPricingSequence');
        savePriLink.append(saveLinkImg);

        var deleteEditImg=$('<img>').attr('src','/resources/images/delete-edit-row.png');
        var closeEditLink=$('<a>').attr('href',"").addClass('closeEditRowLink hidden').append(deleteEditImg);

        var editImgRow=$('<img>').attr('src','/resources/images/editRow.png');
        var editPricingSeqPricingTypeRow=$('<a>').attr('href',"").addClass('editPricingSeqPricingTypeRow').append(editImgRow);

        var deleteImgRow=$('<img>').attr('src','/resources/images/deleteRow.png');
        var deleteRowLink=$('<a>').attr('href',"").addClass('deleteRowLink deletePricingSequence').append(deleteImgRow);
        body.append(unidEle);
        unidEle.append(expandRowLinkEle).append(collapseRowLinkEle).append(collapseRowLinkEle).append(unidSpan);

        body.append(descriptionEle);
        body.append(lastChild);
        lastChild.append(savePriLink).append(closeEditLink).append(editPricingSeqPricingTypeRow).append(deleteRowLink);
                    ...
                    success : function(str) {
                        ...
                        $
                                .each(
                                        ...
                                        function() {
                                            var dataRow = $('<tr>');
                                            dataRow.attr("id", 'pricingTypeId_'
                                                    + this['operationId'] + "_"
                                                    + seqId);
                                            dataRow
                                                    .addClass('pricingSeqOperation');
                                            dataRow.attr("seqId",
                                                    'pricingSeqId_' + seqId);
                                            dataRow.attr("newRow",
                                                    'false');
                        ...
                        var dragRowHtml = "<tr class='placeholder'><td class='dragDropTitle' colspan='6'>Drag and drop pricing types from the pricing table above.</td></tr>";
    ...
    function loadGroupComboData(params) {
        ...
        jQuery.ajax({
            ...
            success: function(str){
                ...
                $('#totalGroupCombinations').html("Group Combinations - showing "+str.groupCombos.groupCombos.length+' of '+str.groupCombos.totalRecords);
                var totalPages = str.groupCombos.totalRecords/groupComboRecordsPerPage;
    ...
    function loadUIGroupComboData(groupCombo) {
        var unid = typeof(groupCombo["unid"]) != 'undefined' ? groupCombo["unid"] : "";
        var description = typeof(groupCombo["description"]) != 'undefined' ? groupCombo["description"] : "";
        var type = typeof(groupCombo["type"]) != 'undefined' ? groupCombo["type"] : "";
        var sequence = typeof(groupCombo["sequence"]) != 'undefined' ? groupCombo["sequence"] : "";
        var comboGroupIds = typeof(groupCombo["comboGroupIds"]) != 'undefined' ? groupCombo["comboGroupIds"] : "";
        var comment =  typeof(groupCombo["commentText"]) != 'undefined' ? groupCombo["commentText"] : "";
        var body = $('<tr>').attr("id", "grpCombo_"+unid);
        var unidEle = $('<td>').addClass('first-child groupComboUnid').append(unid);
        var descriptionEle = $('<td>').addClass('groupComboDescription').append(description);
        var typeEle = $('<td>').addClass('groupComboType').append(type);
        var sequenceEle = $('<td>').addClass('groupComboSequence').append(sequence);
        var commentEle =  $('<td>').attr("style","display: none").addClass('groupComboCommentText').append(comment);
        var comboGroupIdsEle = $('<td>').addClass('groupComboIds').append(comboGroupIds);
        var editImage = $('<img>').attr('src', 'resources/images/editRow.png');
        var deleteImage = $('<img>').attr('src', 'resources/images/deleteRow.png');
        var editEleContent = $('<a>').attr('href', "").addClass("editRowLink editGroupCombo").append(editImage);
        var deleteEleContent = $('<a>').attr('href', "").addClass("deleteRowLink deleteGroupCombo").append(deleteImage);
        var lastEle = $('<td>').addClass('last-child').append(editEleContent).append(deleteEleContent);
        body.append(unidEle);
        body.append(descriptionEle);
        body.append(typeEle);
...
/*--- create Group combinations,Create Product Attribute,Create Customers Attribute popups Call ---*/
        $('.createNewGroupCominations').live('click',function(){
            $('#createGroupComboForm div.groupComboTypeDivWrapper').removeClass('jstree-drop-org');// This is needed to remove the class if exists.
            $('#createGroupComboForm div.groupComboTypeDivWrapper').removeClass('jstree-drop-product');
            $('#createGroupComboForm div.groupComboTypeDivWrapper').attr('unid','');
            $('.validationMsg').hide();
            $('#groupComboTypeTable tbody').empty();
            $('#createGroupComboForm')[0].reset();
            $('#createGroupComboForm div.groupComboTypeDivWrapper').addClass('jstree-drop-org');//To add the class after removing.(To avoid the multiple classes of same name)
            $('#createGroupComboForm input#groupOrganization').attr("checked","checked");
    ...
    //  });
     ...
     $('#addlineItemLink').live('click',function(){
         ...
         var newLineItem = "<tr class='lineItemRow'><td class='first-child'>"+productIDNext+"</td><td><input type='text' class='inputText rangeInput'>" +
    ...
    $('#lineItemTable .expandRowLink').live('click',function(){
        ...
        $(this).parents('tr.expandableRow').next('tr.lineItemExpandedRow').attr("style", "");
    ...
    $('#lineItemTable .collapseRowLink').live('click',function(){
        ...
        $(this).siblings('.expandRowLink').removeClass('hidden');
        $(this).parents('tr.expandableRow').removeClass('expandedRow');
        $(this).parents('tr.expandableRow').next('.lineItemExpandedRow').attr("style", "display : none;");
        return false;
    })
    ...
    $('#clearLineItems').live('click', function(){
         var newLineItem = "<tr class='lineItemRow'><td class='first-child'>1</td><td><input type='text' class='inputText rangeInput'></td><td><input type='text' class='inputText rangeInput'/></td><td></td><td></td><td></td><td></td><td></td><td></td><td class='last-child'><input type='text' class='inputText rangeInput'></td></tr>";
    ...
    function loadUITestingData(quote) {
        ...
        var groupId = typeof(quote["groupId"]) != 'undefined' ? quote["groupId"] : "";
        var groupName = typeof(quote["groupName"]) != 'undefined' ? quote["groupName"] : "";
        var customerName = typeof(quote["customerName"]) != 'undefined' ? quote["customerName"] : "";
        var priceDate = typeof(quote["priceDate"]) != 'undefined' ? quote["priceDate"] : "";
        var pricingSeqId = typeof(quote["pricingSeqId"]) != 'undefined' ? quote["pricingSeqId"] : "";
        var unid = typeof(quote["unid"]) != 'undefined' ? quote["unid"] : "";
        var quoteName = typeof(quote["name"]) != 'undefined' ? quote["name"] : "";
        var optionalArgs = typeof(quote["optionalArgs"]) != 'undefined' ? quote["optionalArgs"] : "";
        ...
        $('#quoteFormWrapper .pricingSeq option[value="'+ pricingSeqId +'"]').attr("selected", "selected");
        $('#quoteFormWrapper .lineItemTbody').html("");
        $("#testingQuoteForm input.saveQuote").attr("isnew", false);
        var count = 1;
        $.each(quote.lineItems, function() {
            var productNumber = typeof(this["productNumber"]) != 'undefined' ? this["productNumber"] : "";
            var productDesc = typeof(this["productDesc"]) != 'undefined' ? this["productDesc"] : "";
            var unid = typeof(this["unid"]) != 'undefined' ? this["unid"] : "";
            var productId = typeof(this["productId"]) != 'undefined' ? this["productId"] : "";
            var quantity = typeof(this["quantity"]) != 'undefined' ? this["quantity"] : 0;
            var price = typeof(this["price"]) != 'undefined' ? this["price"] : 0;
            var optionalArgs = typeof(this["optionalArgs"]) != 'undefined' ? this["optionalArgs"] : "";
            ...
            var inputPr = $('<input>').addClass('inputText rangeInput').val(productNumber);
            ...
            var inputOpArg = $('<input>').addClass('inputText rangeInput').val(optionalArgs);
            var OpArgCol = $('<td>').append(inputOpArg);

            row.append(idCol).append(quantityCol).append(productCol).append(prodNumCol).append(operandCol);
            row.append(amountCol).append(resultCol).append(unitPrCol).append(totalPrCol).append(OpArgCol);
    ...
    function loadUILineItems(lineItems) {
        ...
        $.each(lineItems, function() {
            var productNumber = typeof(this["productNumber"]) != 'undefined' ? this["productNumber"] : "";
            var unid = typeof(this["productID"]) != 'undefined' ? this["productID"] : "";
            var quantity = typeof(this["quantity"]) != 'undefined' ? this["quantity"] : 0;
            var price = typeof(this["price"]) != 'undefined' ? this["price"] : 0;
            var optionalArgs = typeof(this["optionalArgs"]) != 'undefined' ? this["optionalArgs"] : "";
            var row = $('<tr>').addClass('expandableRow lineItemRow');
            ...
            var idCol = $('<td class="first-child"><a href="" class="expandRowLink"><img src="resources/images/expand-row.png"></a><a href="" class="collapseRowLink hidden"><img src="resources/images/collapse-row.png"></a>').append(unid);
            var inputQ = $('<input>').addClass('inputText rangeInput').val(quantity);
            var quantityCol = $('<td>').append(inputQ);
            ...
            if(pricingTypes.length > 0) {
                var count = 1;
                var lineItemsAttr = "<tr class='lineItemExpandedRow' style='display:none;'><td colspan='3' class='first-child'></td><td colspan='6' style='padding:0px'><table class='lineItemExpandedTable' id=row_"+unid+"></table></td><td class='last-child'></td></tr>";
                ...
                $.each(pricingTypes, function() {
                    ...
                    var operand = typeof(this["operand"]) != 'undefined' ? this["operand"] : 0;
                    var amount = typeof(this["amount"]) != 'undefined' ? this["amount"] : 0;
                    var result = typeof(this["result"]) != 'undefined' ? this["result"] : 0;
                    var subRow = $("<tr>").addClass(rowClass);
                    var descriptionEle = $('<td>').addClass("rowHeader descriptionTD first-child").append(description);
                    ...
                    var totalPriceEle = $('<td>').addClass("totalPriceTD last-child").append("-");
                    subRow.append(descriptionEle).append(operandEle).append(amountEle).append(resultEle).append(unitPriceEle).append(totalPriceEle);
    ...
    function getQuoteParams() {
        ...
        quoteGenerateTime = d.getDate()+'-'+d.getMonth()+'-'+d.getFullYear()+'_'+timeStamp;
        ...
        var href=$('.viewTraceImage a').attr("href");
        href = href.split("?");
        href = href[0]+"?filename=pricequote-"+unid+"-"+$(".userName").text()+"-"+quoteGenerateTime+".txt";
        $('.viewTraceImage a').attr("href", href);
        return params;
    }
    ...
    function getAllLineItems() {
        var dataRows = [];
        $('#testingQuoteForm tbody.lineItemTbody tr.lineItemRow').each(function (index, value) {
    ...
    function getLineItemRow(rowNum) {
        var tableRow = $('#testingQuoteForm tbody.lineItemTbody tr.lineItemRow').eq(rowNum);
        var row = {};
...
/*    $("#viewTraceBtn").live('click', function(){
    ...
    $('#addPricingPreferenceLink').live('click',function(){
        ...
        var element = $('#pricingPrefBody tr');
         var tagId = element.last().find('td:eq(0) select.prefTypeDropDown').attr("id")
        var tags = tagId.split('-');
        var productIDLast = tags[1];
         if(productIDLast){
             productIDNext=parseInt(productIDLast)+1;
         }
        var prefernceAdded = "<tr class='newPrefRow' id='pricingPref-"+productIDNext+"'><td><select class='inputText popTableSelect prefTypeDropDown'" +
                        " id='preferenceTypeDD-"+productIDNext+"'><option selected='selected' value='-1'>Choose type</option>" +
                        "</td><td><select class='inputText popTableSelect prefIdDropDown' id='preferencesIdDD-"+productIDNext+"'><option selected='" +
                        "selected' value='-1'>Choose Pref ID</option></select></td><td>" +
                        "<select class='inputText popTableSelect prefNameDropDown' id='preferencesNameDD-"+productIDNext+"'>" +
        ...
        $('#pricingPrefData').children('div').each(function(){
            var o = new Option($(this).attr("id"),$(this).attr("unid"));
            $('#preferenceTypeDD-'+productIDNext).append(o);
        });
        $('#pricingPreferencePopup .popupFormField').animate({scrollTop: $('#pricingPreferencePopup .popupFormField table').height()}, 'fast');
        return false;
    });
    ...
    function getPrefParams() {
        ...
        $('#pricingPrefBody tr').each(function (index, value) {
            ...
            if(!(currentRow.type == "-1" || currentRow.prefId == "-1" || currentRow.name == "-1"))
                dataRows.push(JSON.stringify(currentRow));
        });
        return dataRows;
    }
    ...
    function getPreferenceRow(rowNum) {
        ...
        var row = {};
        row.type = tableRow.find('td:eq(0) select.prefTypeDropDown option:selected').val();
        row.name = tableRow.find('td:eq(1) select.prefIdDropDown option:selected').val();
        row.pref_id = tableRow.find('td:eq(2) select.prefNameDropDown option:selected').val();
    ...
    function loadPreferences(preferences) {
        ...
        $.each(preferences, function() {
            ...
            var prefValue = typeof(this["prefValue"]) != 'undefined' ? this["prefValue"] : "";
            var type = typeof(this["type"]) != 'undefined' ? this["type"] : "";

            var body = $('<tr>').attr("id", "pricingPref-"+unid);

            var typeDropDownEle = $('<select class="inputText popTableSelect prefTypeDropDown">').attr("id", "preferenceTypeDD-"+unid);
            typeDropDownEle.append("<option>Choose type</option>");
            var selectedTypeId = "";
            $('#pricingPrefData').children('div').each(function(){
                var o = $('<option>').attr("value", $(this).attr("unid")).append($(this).attr("id"));
            ...
            var prefIdDropDownEle = $('<select class="inputText popTableSelect prefIdDropDown">').attr("id", "preferencesIdDD-"+unid);
            prefIdDropDownEle.append("<option>Choose Pref ID</option>");
            var prefNameDropDownEle = $('<select class="inputText popTableSelect prefNameDropDown">').attr("id", "preferencesNameDD-"+unid);
            prefNameDropDownEle.append("<option>Choose Pref</option>");
            $('#'+selectedTypeId).children('span').each(function(){
                var o = $('<option>').attr("value", $(this).attr("unid")).append($(this).text());
                var obj = $('<option>').attr("value", $(this).text()).append($(this).attr("unid"));
            ...
            var inputEle = $('<input type="text">').addClass("inputText popTableInputText").val(prefValue);
            var prefValueEle = $('<td>').append(inputEle);
            var unidEle = $('<td>').append(unid);
            var deleteEle = $('<td>').append('<a href="#" class="deletePricingPref"><img src="resources/images/deleteRow.png" alt="Delete"/></a>');

            body.append(typeEle).append(prefIdEle).append(nameEle).append(prefValueEle);
    ...
    $('.deleteToken').live('click',function(){
        ...
        var unid = $(this).parents('.tokenList').siblings('.tokenInputText').attr('unid');
        var itemIndex = unid.search(itemId);
        if (itemIndex+itemId.length == unid.length && itemId.length != unid.length ) {
            unid = unid.replace(':'+itemId,'');
        } else if (itemIndex+itemId.length==unid.length && itemId.length==unid.length) {
            unid = unid.replace(itemId,'');
        }  else {
            unid = unid.replace(itemId+':','');
        }
        ...
        $(this).parents('.tokenList').siblings('.tokenInputText').attr('unid',unid);
        $(this).parents('li').remove();
        return false;
    });
    ...
    $('#configurationBtn').live('click',function(){
        if($('.confTab1').parents('li').hasClass('active')){
            var showTableIdTreeCheck=$('#showTableIdTreeCheck').is(':checked') ? 'checked' : 'unchecked';
            var showTableIdDialogCheck=$('#showTableIdDialogCheck').is(':checked') ? 'checked' : 'unchecked';
            var effDateFrom=$('#effDateFrom').val();
            var effDateTo=$('#effDateTo').val();
            $('#defStartDate').html(effDateFrom);
            $('#defEndDate').html(effDateTo);

            addLayoutConfiguration(showTableIdTreeCheck+","+showTableIdDialogCheck+","+effDateFrom+","+effDateTo,
                    "TABLE_UNIQUE_ID_IN_TREE,TABLE_UNIQUE_ID_IN_DIALOGS,EFF_DATE_FROM,EFF_DATE_TO","/configuration/update");
            showTblIdTreeCheck=$('#defaultConfigurationData').children('#showTableIdTreeCheckDefault').text();
            $('#defaultConfigurationData').children('#showTableIdTreeCheckDefault').text($('#showTableIdTreeCheck').is(':checked') ? 'checked' : 'unchecked');
            $('#defaultConfigurationData').children('#effDateFromDefault').text(effDateFrom);
            $('#defaultConfigurationData').children('#effDateToDefault').text(effDateTo);
            $('#defaultConfigurationData').children('#showTableIdDialogCheckDefault').text($('#showTableIdDialogCheck').is(':checked') ? 'checked' : 'unchecked');
           // if(showTblIdTreeCheck!=showTableIdTreeCheck){
            ...
            var productsMasterNameCheck=$('#getProductsMasterNameCheck').is(':checked')  ? 'checked' : 'unchecked';
            addLayoutConfiguration(productsMasterNameCheck,"GET_PRD_MASTER_NAME","/configuration/update");
            $('#defaultConfigurationData').children('#getProductsMasterNameCheckDefault').text($('#getProductsMasterNameCheck').is(':checked') ? 'checked' : 'unchecked');
         }
         $('.pageOverlay').fadeOut('fast');
         $('.popupWrapper').fadeOut('fast');
    });
    ...
    $('#engineTraceFile').live('click',function(){
     ...
     $("#deletePricingSeqsPType").live('click',function(){
         ...
         if (newRow == 'false' && typeof parentId !== 'undefined' && parentId !== false) {
    ...
    $('.deletePricingTypeRow').live(
            ...
            function() {
                ...
                $('span#newRow').text($(this).parents('tr.pricingSeqOperation').attr('newRow'));
                $('span#pricingSequencePTypeUnid').text(
                        $(this).parents('tr.pricingSeqOperation').attr('id')
                                .split('_')[1]);
                if (typeof seqIdAttr !== 'undefined' && seqIdAttr !== false) {
                    $('span#parentPricingSequenceUnid').text($(this).parents('tr.pricingSeqOperation').attr('seqId').split('_')[1]);
                }
                return false;
            });
    ...
    function getAttrDatas(formId,attrCls){
        var dataRows=[];
        $('#'+formId+' tbody.'+attrCls+' tr.attrRow').each(function(index,value){
    ...
    function getAttrRow(rowNum,formId,attrCls){
        var tableRow=$('#'+formId+' tbody.'+attrCls+' tr.attrRow').eq(rowNum);
        var row={};
    ...
    $('.chooseColBtn').live('click',function(){
    	...
    	$('#choosecolumn #columnNotDisplayed option').each(function(){    		
    		...
    		tblID=$('#'+$(this).val()).parents('.recordsTable').attr('id');
    		$('#'+tblID+' td:nth-child('+colId+'),#'+tblID+' th:nth-child('+colId+')').hide();
    		var o = new Option($(this).text(),$(this).val());
    	...
    	$('#choosecolumn #columnDisplayed option').each(function(){    		
    		...
    		colId=getColumnIndexesById($(this).val());
     		tblID=$('#'+$(this).val()).parents('.recordsTable').attr('id');
    		$('#'+tblID+' td:nth-child('+colId+'),#'+tblID+' th:nth-child('+colId+')').show();
    		var o = new Option($(this).text(),$(this).val());
...
function validation(formId){
    ...
    if(formId == 'createAdjustment' || formId == 'editAdjustment'){
        if($('#'+formId+' .adjCustId').val() == '' && $('#'+formId+'  .adjOrgs').attr('unid') == ''){
            isValid = false;
            $('.adjOrgs').parent().siblings('#'+formId+' .validationMsgMultiple').show();
        }
        if ($('#'+formId+' input.adjProdId').val() == '' && $('#'+formId+' input.adjProdGroups').attr('unid') == '') {
            isValid = false;
            $('.adjProdGroups').parent().siblings('.validationMsgMultiple').show();
        };
    }
    if((formId == 'createProductAttributeForm') || (formId == 'customerAttributeForm')) {
        if(!($('.optionOne').is(':checked')) && !($('.optionTwo').is(':checked'))) {
            isValid = false;
            $('#'+formId+' .optionTwo').parent().siblings('.validationMsg').css('display','inline-block');
        }
    }
    ...
    if (startDate > endDate){
        isValid = false;
        $('#'+formId+' input.startDate').siblings('.validationMsg').show();
    }
    ...
    $( ".customer-id-drop" ).droppable({
                ...
                drop: function( event, ui ) {
                    if($(this).hasClass('adjCustId')){
                        ...
                        var li = $('<li>').attr('itemid',$(this).text()).text(ui.draggable.children('.customer-id').text());
                        var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                        li.append(an);
                        $('#'+formId+' ul#customerId').append(li);
                        $(this).parents('tr').siblings('.customer-name-field-row').children('.customer-name').text(ui.draggable.children('.customer-name').text());
                    }else if($(this).hasClass('proSrcCustId')){
                        $('#prodAdvanceSearch .proSrcCustId').siblings('ul.tokenList').empty();
                        var li = $('<li>').attr('itemid',$(this).text()).text(ui.draggable.children('.customer-id').text());
                        var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                        li.append(an);
                        $('#prodAdvanceSearch .proSrcCustId').siblings('ul.tokenList').append(li);
                        $(this).val(ui.draggable.children('.customer-id').text());
                        $('#proSrcCustName').text(ui.draggable.children('.customer-name').text());
                    }else if($(this).hasClass('adjSerCustId')){
                        $('#adjAdvanceSearch .adjSerCustId').siblings('ul.tokenList').empty();
                        var li = $('<li>').attr('itemid',$(this).text()).text(ui.draggable.children('.customer-id').text());
                        var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                        li.append(an);
                        $('#adjAdvanceSearch .adjSerCustId').siblings('ul.tokenList').append(li);
                        $(this).val(ui.draggable.children('.customer-id').text());
                        $('#advSearchCustName').text(ui.draggable.children('.customer-name').text());
                    }else if($(this).hasClass('customerGroupId')){
                        $('.customerGroupId').siblings('ul.tokenList').empty();
                        var li = $('<li>').attr('itemid',ui.draggable.children('.customer-id').text()).text(ui.draggable.children('.customer-id').text());
                        var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                        li.append(an);
                        $('.customerGroupId').siblings('ul.tokenList').append(li);
                        $(this).val(ui.draggable.children('.customer-id').text());
                        $('.customerNameVal').text(ui.draggable.children('.customer-name').text());
                    }
                }
            });
        $( ".product-id-drop" ).droppable({
                ...
                drop: function( event, ui ) {
                    if($(this).hasClass('adjProdId')){
                        ...
                        var li = $('<li>').attr('itemid',ui.draggable.children('.product-name').parent().attr('id').substr('5')).text(ui.draggable.children('.product-name').parent().attr('id').substr('5'));
                        var an = $('<a>').attr('href','').addClass('deleteToken').append($('<img>').attr('src','resources/images/deleteInputToken.png'));
                        li.append(an);
                        $('.adjProdId').siblings('ul.tokenList').append(li);
                        $(this).val(ui.draggable.children('.product-name').parent().attr('id').substr('5'));
                    } else {
...
function popupWindow(url){
    ...
    window.open(
        popUpUrl,'popUpWindow'+popUpUrl,'height=700,width=1000,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes'
        )
}
...
function popAlert(message){ // to display custome alert popup
    $('#alertPopup .alertMessageDiv').text(message);
    $('.pageOverlay').fadeIn('fast');
    $('#alertPopup').fadeIn('fast');
}

function popSuccessMessage(message){ // to fade in the success message/ notification in inner pages and fadeout
